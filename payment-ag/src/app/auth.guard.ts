...
export class AuthGuard implements CanActivate {
  ...
  canActivate(route: any, state: unknown): boolean {
    ...
    if (!userRole) {
      this.router.navigate(['payment/loginpage']); // Redirect if not logged in
      return false;
    }

    // 🔹 Define role-based allowed routes
    const allowedAdminRoutes = ['payment/mnusrpage', 'payment/pymntpage'];
    const allowedUserRoutes = ['payment/pymntpage'];
    ...
    if (!isRouteAllowed) {
      this.router.navigate(['payment/pymntpage']); // Redirect users to payment page
      return false;
    }
